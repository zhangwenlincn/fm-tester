...
/// 写入环境配置
pub fn write_environments_config(workspace_path: &str, config: &EnvironmentsConfig) -> Result<(), String> {
    let path = get_environments_config_path(workspace_path);
...
/// 保存环境（创建或更新）
#[tauri::command]
pub fn save_environment(workspace_path: String, environment: Environment) -> Result<Environment, String> {
    let mut config = read_environments_config(&workspace_path);
    
    // 查找是否存在相同 id 的环境
    let existing = config.environments.iter_mut().find(|e| e.id == environment.id);
...
/// 替换字符串中的环境变量 {{变量名}}
pub fn replace_variables(text: &str, variables: &std::collections::HashMap<String, String>) -> String {
